/**
 * Transcript Parser
 * Ported from the Flask backend Python code
 ...
 * This implements the same logic as the Python parse_transcript function
 */
...
/**
 * Extract text from left and right columns of PDF pages
 * This mimics the two-column layout of AUIS transcripts
 * Note: This function expects already-extracted text with column markers
 */
export function combineColumnText(
  leftTextBlocks: string[],
  rightTextBlocks: string[]
): string {
  // Concatenate all left columns, then all right columns
  return [...leftTextBlocks, ...rightTextBlocks].join("\n");
}
